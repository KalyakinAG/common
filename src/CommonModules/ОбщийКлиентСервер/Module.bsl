...
Функция ПредставлениеСтруктуры(Структура) Экспорт
	Поля = РаботаСМассивом.Отобразить(Структура, "СтрШаблон('%1: """"%2""""', Элемент.Ключ, Строка(Элемент.Значение))");
	Представление = "{
...
Функция СвойствоСтруктурыРекурсивно(Приемник, ПутьКДанным, ЗначениеПоУмолчанию = Неопределено, УстановитьЗначение = Ложь) Экспорт
	Перем ПодчиненныйУзел;
	Перем Значение;
	СоставПути = СтрРазделить(ПутьКДанным, ".");
	Узел = Приемник;
	...
	КонецЦикла;
	Свойство = СоставПути[СоставПути.ВГраница()]; 
	Если НЕ Узел.Свойство(Свойство, Значение) Тогда
		Если УстановитьЗначение Тогда
			Узел.Вставить(Свойство, ЗначениеПоУмолчанию);
		КонецЕсли;
		Возврат ЗначениеПоУмолчанию;
	КонецЕсли;
	Возврат Значение;
...
Функция СвойствоСоответствияРекурсивно(Приемник, ПутьКДанным, ЗначениеПоУмолчанию = Неопределено, УстановитьЗначение = Ложь) Экспорт
	Перем ПодчиненныйУзел;
	СоставПути = СтрРазделить(ПутьКДанным, ".");
	Узел = Приемник;
	Для Индекс = 0 По СоставПути.ВГраница() - 1 Цикл
...
Функция СвойствоРекурсивно(Приемник, ПутьКДанным, ЗначениеПоУмолчанию = Неопределено, УстановитьЗначение = Ложь) Экспорт
	Если ТипЗнч(Приемник) = Тип("Соответствие") Тогда
		Возврат СвойствоСоответствияРекурсивно(Приемник, ПутьКДанным, ЗначениеПоУмолчанию, УстановитьЗначение);
	ИначеЕсли ТипЗнч(Приемник) = Тип("Структура") Тогда
		Возврат СвойствоСтруктурыРекурсивно(Приемник, ПутьКДанным, ЗначениеПоУмолчанию, УстановитьЗначение);
	ИначеЕсли ТипЗнч(Приемник) = Тип("КлючИЗначение") Тогда
		СоставПути = СтрРазделить(ПутьКДанным, ".");
		Поле = РаботаСМассивом.Сдвинуть(СоставПути);
		Если НЕ ЗначениеЗаполнено(СоставПути) Тогда
			Если УстановитьЗначение И ВРег(Поле) = "ЗНАЧЕНИЕ" Тогда
				Значение = Приемник.Значение;
				Если НЕ ЗначениеЗаполнено(Значение) Тогда
					Приемник.Значение = ЗначениеПоУмолчанию;
					Возврат ЗначениеПоУмолчанию;
				КонецЕсли;
				Возврат Значение;
			КонецЕсли;
			Возврат Приемник[Поле];
		КонецЕсли;
		Возврат СвойствоРекурсивно(Приемник[Поле], СтрСоединить(СоставПути, "."), ЗначениеПоУмолчанию, УстановитьЗначение);
	Иначе
		ВызватьИсключение "Неверный тип приемника";
	КонецЕсли;
КонецФункции

Функция Свойство(Приемник, Свойство, ЗначениеПоУмолчанию = Неопределено, УстановитьЗначение = Ложь) Экспорт
	Если ТипЗнч(Приемник) = Тип("Соответствие") Тогда
		Значение = Приемник[Свойство];
		Если Значение = Неопределено Тогда
			Если УстановитьЗначение Тогда
				Приемник.Вставить(Свойство, ЗначениеПоУмолчанию);
			КонецЕсли;
			Возврат ЗначениеПоУмолчанию;
		КонецЕсли; 
		Возврат Значение;
	ИначеЕсли ТипЗнч(Приемник) = Тип("Структура") Тогда
		...
		Если НЕ Приемник.Свойство(Свойство, Значение) Тогда
			Если УстановитьЗначение Тогда
				Приемник.Вставить(Свойство, ЗначениеПоУмолчанию);
			КонецЕсли;
			Возврат ЗначениеПоУмолчанию;
...
// 
// Параметры:
//  Элемент - Структура - составной объект
//  СоответствиеСвойств - Строка - Соответствие свойств в формате "ПолеПриемника = ПолеИсточника, ...". Для сохранения 
//  имен свойств ввести имя свойства без соответствия. Имя "*" для копирования всех свойств источника в приемник.
// 
// Возвращаемое значение:
//  Структура - Отобразить свойства
Функция ОтобразитьСвойства(Элемент, СоответствиеСвойств) Экспорт
	Результат = Новый Структура;
	СписокСоответствий = РаботаСМассивом.Массив(СоответствиеСвойств);
	Для Каждого ЭлементСоответствия Из СписокСоответствий Цикл
		СоставСоответствия = СтрРазделить(ЭлементСоответствия, "= ", Ложь);
		Если СоставСоответствия.Количество() = 2 Тогда
			ПолеПриемника = СоставСоответствия[0];
			ПолеИсточника = СоставСоответствия[1];
			Результат.Вставить(ПолеПриемника, СвойствоРекурсивно(Элемент, ПолеИсточника));
			Продолжить;
		КонецЕсли;
		ПолеПриемника = СоставСоответствия[0];
		Если ПолеПриемника = "*" Тогда
			Для Каждого ЭлементСвойства Из Элемент Цикл
				Результат.Вставить(ЭлементСвойства.Ключ, ЭлементСвойства.Значение);
			КонецЦикла;
			Продолжить;
		КонецЕсли;
		Результат.Вставить(ПолеПриемника, Элемент[ПолеПриемника]);
	КонецЦикла;
	Возврат Результат;
КонецФункции

// Сгруппировать массив структур в соответствие или структуру. Поле значения будет представлено
// в виде массива
// 
// Параметры:
//  Источник - Массив из Структура - Источник:
// * ПолеГруппировки - Строка - 
// * ПолеЗначения - Строка - 
//  ПолеГруппировки - Строка - Поле группировки
//  ПолеЗначения - Строка - Поле массива сгруппированных элементов
//  ВСоответствие - Булево - В соответствие
// 
// Возвращаемое значение:
// - Структура из Произвольный, 
// - Соответствие из Произвольный
Функция СгруппироватьМассив(Источник, ПолеГруппировки, ПолеЗначения, ВСоответствие = Истина) Экспорт
	Результат = РаботаСМассивом.Преобразовать(
		Источник, 
		СтрШаблон("ОбщийКлиентСервер.ВставитьСвойство(
			|НачальноеЗначение, Элемент.%1, 
			|	Добавить(ОбщийКлиентСервер.Свойство(НачальноеЗначение, Элемент.%1, Новый Массив), Элемент.%2)
			|)", ПолеГруппировки, ПолеЗначения
		), 
		?(ВСоответствие, Новый Соответствие, Новый Структура)
	);
	Возврат Результат;		
КонецФункции
...
Функция ПрочитатьЗначениеJSONИзФайла(ИмяФайла, Знач ИменаСвойствСоЗначениямиДата = Неопределено, Знач ПрочитатьВСоответствие = Истина) Экспорт
	ЧтениеJSON = Новый ЧтениеJSON;
...
Функция ЭтоJSON(СтрокаJSON) Экспорт
	Если ТипЗнч(СтрокаJSON) <> Тип("Строка") Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат ЭтоСтруктураJSON(СтрокаJSON) ИЛИ ЭтоМассивJSON(СтрокаJSON);
КонецФункции
...
Функция ЭтоСтруктураJSON(СтрокаJSON) Экспорт
	Если ТипЗнч(СтрокаJSON) <> Тип("Строка") Тогда
		Возврат Ложь;
	КонецЕсли;
	Позиция = СтрНайти(СтрокаJSON, "{");
	Если Позиция > 0 И ПустаяСтрока(Лев(СтрокаJSON, Позиция - 1)) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
